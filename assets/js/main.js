...
function applyDiscountToAllProducts(discountPercent = 45) {
    const discountRate = (100 - discountPercent) / 100;
    Object.values(productDetails).forEach(products => {
        products.forEach(product => {
            product.variants.forEach(variant => {
                if (!variant.price || typeof variant.price !== 'string' || variant.price.toLowerCase().includes('liên hệ')) return;

                const numericPrice = parseInt(variant.price.replace(/\./g, '').replace(' VND', ''), 10);
                if (isNaN(numericPrice)) return;

                const discounted = Math.round(numericPrice * discountRate);
                variant.discounted_price = `
    <span style="color: green; font-weight: bold;">
        ↓ Giá sau giảm: ${discounted.toLocaleString('vi-VN')} VND
    </span>
`;
...
applyDiscountToAllProducts(45);
    ...
    function createProductCardHtml(product) {
        ...
        if (product.variants && product.variants.length > 0) {
            product.variants.forEach(variant => Object.keys(variant).forEach(key => allKeys.add(key)));
        }
        const headers = Array.from(allKeys);
        const isDimensionColumn = (header) => header.toLowerCase().includes('kích thước') || header.toLowerCase().includes('dimensions');
        const createTableRow = (variant) => headers.map(header => `<td class="px-3 py-2 border-b border-gray-200 whitespace-nowrap ${isDimensionColumn(header) ? 'dimensions-column' : ''}">${variant[header] || 'N/A'}</td>`).join('');
        const createMobileCard = (variant, index) => `<div class="bg-gray-50 rounded-lg p-3 border border-gray-200"><p class="text-red-700 text-sm font-semibold mb-2">Phiên bản ${index + 1}</p><dl class="space-y-1">${headers.map(header => `<div class="flex justify-between text-xs ${isDimensionColumn(header) ? 'dimensions-column' : ''}"><dt class="text-gray-500 font-medium">${header}</dt><dd class="font-semibold text-right ${isDimensionColumn(header) ? '' : 'text-gray-800'}">${variant[header] || 'N/A'}</dd></div>`).join('')}</dl></div>`;
        return `<div class="product-card bg-white p-4 rounded-lg mb-6 shadow-sm border border-gray-200"><div class="flex flex-col md:flex-row items-start gap-5 mb-4"><img src="${product.image || 'https://placehold.co/300x300/e2e8f0/64748b?text=MAXBEN'}" alt="${product.name}" class="w-full md:w-40 h-40 object-contain flex-shrink-0 rounded-md border border-gray-200 bg-gray-50" onerror="this.onerror=null; this.src='https://placehold.co/300x300/e2e8f0/64748b?text=MAXBEN';"><div class="flex-1"><p class="text-lg font-bold text-gray-800 mb-2">${product.name}</p><p class="text-sm text-gray-600 mb-2"><strong>Mô tả:</strong> ${product.description || 'Chưa có thông tin.'}</p><p class="text-sm text-gray-600 mb-3"><strong>Ứng dụng:</strong> ${product.applications || 'Chưa có thông tin.'}</p><div class="text-sm text-gray-600"><p class="font-semibold mb-1"><strong>Đặc điểm nổi bật:</strong></p><ul class="list-disc list-inside text-sm space-y-1 pl-4">${product.features ? product.features.split('. ').filter(f => f.trim() !== '').map(feature => `<li>${feature.trim()}</li>`).join('') : '<li>Chưa có thông tin.</li>'}</ul></div></div></div>${headers.length > 0 ? `<div class="overflow-x-auto hidden md:block"><table class="product-table w-full text-sm"><thead class="bg-red-100"><tr>${headers.map(h => `<th class="px-3 py-2 border-b border-gray-300 text-left whitespace-nowrap ${isDimensionColumn(h) ? 'dimensions-column' : ''}">${h}</th>`).join('')}</tr></thead><tbody>${product.variants.map(v => `<tr>${createTableRow(v)}</tr>`).join('')}</tbody></table></div><div class="md:hidden flex flex-col gap-3 mobile-table-card">${product.variants.map(createMobileCard).join('')}</div>` : '<p class="text-sm text-center text-gray-500 mt-4">Chưa có thông số chi tiết.</p>'}</div>`;
    }

    function displayProducts(productList, title) {
        productDetailsContent.innerHTML = `<h4 class="text-xl font-semibold text-gray-800 mb-4">${title}</h4>` + (productList && productList.length > 0 ? productList.map(createProductCardHtml).join('') : `<p class="text-center text-gray-500 py-10">Không tìm thấy sản phẩm phù hợp.</p>`);
    }

    document.querySelectorAll('.product-category-button').forEach(button => button.addEventListener('click', e => {
        const category = e.target.dataset.category;
        displayProducts(productDetails[category] || [], `Danh mục: ${category}`);
        searchInput.value = '';
    }));
...
searchInput.addEventListener('input', () => {
    ...
    // ===================================================================
    // LOGIC BIỂU ĐỒ (CHARTS)
    // ===================================================================
   let productCategoryChart, criComparisonChart;
    ...
    let currentChartState = {
        level: 'main',
        mainCategory: '',
        subProductLabels: [] // Lưu trữ nhãn không bị cắt ngắn của biểu đồ con
    };
    ...
    function initOrUpdateMainChart(labels, data, title) {
        mainChartTitle.textContent = title;
        ...
        productCategoryChart = new Chart(categoryCtx, {
            ...
            options: {
                ...
                onClick: (event, elements) => {
                    ...
                    if (currentChartState.level === 'main') {
                        const mainCategoryName = productCategoryChart.data.labels[clickedIndex];
                        const subProducts = productDetails[mainCategoryName];
                        if (subProducts && subProducts.length > 0) {
                            currentChartState.level = 'sub';
                            currentChartState.mainCategory = mainCategoryName;
                            currentChartState.subProductLabels = subProducts.map(p => p.name); // Lưu tên đầy đủ
                            
                            const subLabels = subProducts.map(p => wrapLabel(p.name, 15));
                            const subData = subProducts.map(p => p.variants.length);
                            initOrUpdateMainChart(subLabels, subData, `Chi tiết: ${mainCategoryName}`);
                            backButton.classList.remove('hidden');
                        }
                    } else if (currentChartState.level === 'sub') {
                        const subProducts = productDetails[currentChartState.mainCategory];
                        const clickedProductName = currentChartState.subProductLabels[clickedIndex]; // Lấy tên đầy đủ
                        const clickedProduct = subProducts.find(p => p.name === clickedProductName);

                        if (clickedProduct) {
                            displayProducts(subProducts, `Danh mục: ${currentChartState.mainCategory}`);
                            setTimeout(() => {
                                const productCard = document.getElementById(`product-${clickedProduct.id}`);
                                if (productCard) {
                ...
                plugins: { 
                    ...
                    tooltip: { backgroundColor: '#111827', titleColor: '#fff', bodyColor: '#fff', padding: 10, cornerRadius: 4 } 
                },
                scales: {
                    y: { beginAtZero: true, grid: { color: chartGridColor }, ticks: { color: chartFontColor } },
                    x: { grid: { display: false }, ticks: { color: chartFontColor, maxRotation: 45, minRotation: 0 } }
                }
            }
        });
    }
    ...
    backButton.addEventListener('click', () => {
        currentChartState.level = 'main';
        const mainCategoryData = Object.keys(productDetails).map(cat => ({ name: cat, count: productDetails[cat].length }));
        initOrUpdateMainChart(mainCategoryData.map(c => c.name), mainCategoryData.map(c => c.count), 'Phân Loại Sản Phẩm Chính');
        backButton.classList.add('hidden');
    });

    const initialCategoryData = Object.keys(productDetails).map(cat => ({ name: cat, count: productDetails[cat].length }));
    initOrUpdateMainChart(initialCategoryData.map(c => c.name), initialCategoryData.map(c => c.count), 'Phân Loại Sản Phẩm Chính');

    const criCtx = document.getElementById('criComparisonChart')?.getContext('2d');
    if (criCtx) {
        const criCounts = { '>95': 5, '>90': 12, '>80': 25 };
        criComparisonChart = new Chart(criCtx, {
            type: 'pie',
            data: {
                labels: [`CRI > 95`, `CRI > 90`, `CRI > 80`],
                datasets: [{
                    label: 'Số Lượng Sản Phẩm',
                    data: [criCounts['>95'], criCounts['>90'], criCounts['>80']],
                    backgroundColor: [brandColors.red, brandColors.redLight, brandColors.redLighter],
                    borderColor: '#fff',
                    borderWidth: 2,
                    hoverOffset: 4
                }]
            ...
            options: {
                responsive: true, maintainAspectRatio: false,
                plugins: {
                    legend: { position: 'bottom', labels: { color: chartFontColor, padding: 20 } },
                    tooltip: { backgroundColor: '#111827', titleColor: '#fff', bodyColor: '#fff', padding: 10, cornerRadius: 4 }
                }
            }
        });
